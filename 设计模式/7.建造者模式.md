...
1. 建房子过程：打桩、砌墙、封顶
2. 房子有各种各样的，如：普通房、高楼、别墅等，各种房子的过程虽然一样，但要求不相同。

### 传统方式

#### 类图

![建房子问题传统方式类图.png](./img/建房子问题传统方式类图.png)
...
1. 建造者模式（Builder Pattern）又叫**生成器模式**，是一种构建模式。可以将复杂对象的建造过程抽象出来（抽象类别），是这个抽象过程的不同实现方法可以构造出不同表现（属性）的对象。
2. 建造者模式是一步一步创建一个复杂的对象，它允许用户只通过指定复杂对象的类型和内容就可以构建它们，用户不需要知道内部的具体构建细节。

#### 建造者模式原理类图

![Builder 模式.png](./img/Builder 模式.png)
...
4. Director（指挥者）：构建一个使用Builder接口的对象。它主要是用于创建一个复杂的对象。主要有两个作用：一：隔离了客户与对象的生产过程；二：负责控制产品对象的生产过程。

#### 示例

##### 类图

![盖房子问题建造者模式类图.png](./img/盖房子问题建造者模式类图.png)
...
7. 抽象工厂模式 VS 建造者模式
   ...
   抽象工厂模式实现对产品家族的创建，一个产品家族是这样的一系列产品；具有不同分类维度的产品组合，采用抽象工厂模式不需要关心构建过程，之关心什么产品由什么工厂生产即可。而建造者模式则是要求按照指定的蓝图建造产品，它的主要目的是通过组装零配件而产生一个新产品。